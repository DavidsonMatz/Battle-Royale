class Client{
	...
	float[] normalize(float[] vector){
		...
		float value = inv_sqrt((vector[0] * vector[0]) + (vector[1] * vector[1]));
		return new float[]{vector[0] * value, vector[1] * value};
	}
	...
	synchronized void send_info(){
		...
		try{
			...
			if(!this.action.equals("update")){
				...
				float[] aux = new float[]{this.normalize(this.vector)[0], this.normalize(this.vector)[1]};
